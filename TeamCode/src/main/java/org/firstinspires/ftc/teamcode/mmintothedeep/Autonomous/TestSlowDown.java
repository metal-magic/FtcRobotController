...
public class TestSlowDown extends LinearOpMode {
    ...
    static final Pose2D TEST_POINT_1 = new Pose2D(DistanceUnit.MM, 500, 100, AngleUnit.DEGREES, 90); // Starting position
        ...
        while (opModeIsActive()) {
            ...
            switch (stateMachine){
                ...
                case DRIVE_TO_TARGET_1:
                    if (nav.driveTo(odo.getPosition(), TEST_POINT_1, 1, 0.2)){
                        telemetry.addLine("at position #1!");
                        stateMachine = StateMachine.DRIVE_TO_TARGET_2;
                    }
                    p = (int) getDistance(odo.getPosition(), TEST_POINT_1);
                    pPower = Math.max(1, p * 0.01);
                    nav.setXYCoefficients(pPower, 0.00001, 20, DistanceUnit.MM, 16);
                    break;
