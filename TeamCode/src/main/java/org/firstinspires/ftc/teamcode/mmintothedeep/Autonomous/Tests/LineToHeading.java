...
@Autonomous(name = "LineToHeading", group = "Autonomous")
@Disabled
public class LineToHeading extends LinearOpMode {
    ...
    /**
     * CONCEPT:
     * moves robot and rotates to heading same time
     * @param x
     * @param y
     * @param headingRelative - assuming current is 0
     */
    public void lineToHeading(double x, double y, double degrees) {

        double currentHeading = 0;
        // Assume positive degrees means moving towards the right
        double movementOfWheelsInRevs = Math.abs(1 / DEGREES_MOTOR_MOVES_IN_1_REV); // moving 1 degree at a time
        double strafeAngleDegrees;
        // tan a = opp / adj = x/y
        double angleStrafe = Math.toDegrees(Math.atan(x/y));
        ...
        if (degrees >= 0) {
            while (currentHeading < degrees) {
                ...
                strafeAngleDegrees = angleStrafe + currentHeading;
                double strafeAngleRadians = Math.PI / 180 * strafeAngleDegrees;
                double strafeAnyRevs = Math.abs(0.1 / CIRCUMFERENCE_INCHES);
                ...
                driveNoDelay(SPEED, strafeLeftFrontPower, strafeLeftBackPower, strafeRightFrontPower, strafeRightBackPower);
            }
        } else {
    ...
    public void driveNoDelay(double speed, double leftFrontRevs, double leftBackRevs, double rightFrontRevs,
    ...
    public void driveWithSlide(double speed, double leftFrontRevs, double leftBackRevs, double rightFrontRevs,
            double rightBackRevs, int slide) {
        ...
        double scale = slide;
        double height = slide;
        while (opModeIsActive() && (tolerance(leftFrontDrive) || tolerance(leftBackDrive) || tolerance(rightFrontDrive)
                || tolerance(rightBackDrive))) {
            ...
            if (!isReached) {
                if (linearSlideMotor.getCurrentPosition() < 4000 && height > linearSlideMotor.getCurrentPosition()) {
                    linearSlideMotor.setPower(scale);
                        ...
                        && height < linearSlideMotor.getCurrentPosition()) {
